        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredProducts.map(product => (
              ...
              >
                <div className="w-full aspect-square rounded-xl bg-background mb-3 overflow-hidden">
                  <img
                    src={product.imageUrl || '/product-images/tacos-barbacoa.svg'}
                    alt={product.name}
      ...
      <div className="w-full lg:w-[400px] flex flex-col bg-card shadow-2xl relative z-10">
          ...
          {cart.length === 0 ? (
            ...
            cart.map(item => (
              <div key={item.id} className="flex items-center gap-3">
                <img
                  src={item.imageUrl || '/product-images/tacos-barbacoa.svg'}
                  alt={item.name}
        ...
        <div className="p-6 bg-background border-t border-muted/10 space-y-4">
          ...
          <div className="space-y-2 pt-2 border-t border-muted/10">
            <p className="text-[10px] font-bold text-muted uppercase tracking-widest">Método de pago</p>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setPaymentMethod('efectivo')}
                className={`flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-sm font-bold border transition-all ${paymentMethod === 'efectivo' ? 'bg-primary text-white border-primary' : 'bg-card text-muted border-muted/20 hover:border-primary/40'}`}
              >
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-sm font-bold border transition-all ${paymentMethod === 'transferencia' ? 'bg-primary text-white border-primary' : 'bg-card text-muted border-muted/20 hover:border-primary/40'}`}
              >
